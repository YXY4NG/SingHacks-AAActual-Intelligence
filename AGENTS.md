...
vocabulary, read `CONTEXT.md` for the canonical domain language, `docs/SPEC.md`
for behaviour, and every relevant decision in `docs/adr/`.

Use the glossary's canonical terms in interfaces, tests, and UI copy. Surface
any conflict with an ADR instead of silently overriding it. General
implementation details do not belong in `CONTEXT.md`.
